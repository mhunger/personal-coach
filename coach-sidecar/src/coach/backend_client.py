...
class BackendClient:
    ...
    async def save_schedule(self, iso_week: str, payload: dict[str, Any]) -> dict[str, Any]:
        return await self._put("/api/schedule", params={"week": iso_week}, json=payload)
    ...
    async def _put(
        ...
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            r = await client.put(f"{self.base_url}{path}", params=params, json=json)
            r.raise_for_status()
            return r.json()
